	...
	public class Query
	{
		...
		public string SecondTask(ref string searchCompanyName)
		{
			string queryString = "SELECT Table1.[Код_предприятия], Table1.[Название_предприятия], Table2.[Название_изделия], " +
									"Table2.[Шифр_изделия], Table2.[Годовой_выпуск], (Table2.[Цена_квартала_1] + Table2.[Цена_квартала_2] + " +
									"Table2.[Цена_квартала_3] + Table2.[Цена_квартала_4]) / 4 AS [Средняя_стоимость_за_год] " +
									"FROM (Table1 INNER JOIN Table2 ON Table1.[Код_предприятия] = Table2.[Код_предприятия]) " +
									$"WHERE (Table1.[Название_предприятия] = '{searchCompanyName}')";
		...
		{
			string queryString = "SELECT Table1.[Код_предприятия], Table1.[Название_предприятия], Table2.[Название_изделия], Table2.[Шифр_изделия], Table2.[Годовой_выпуск], " +
